...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sourceId) {
      ...
      if (result.payment?.status === 'COMPLETED' && savePaymentMethod && session?.user && customerId) {
        try {
          const savedCard = await saveCardOnFile({
            customerId: customerId,
            sourceId: sourceId,
            cardholderName: billingContact?.givenName && billingContact?.familyName
          ...
          console.error('[Square Payment] Failed to save card (payment still succeeded):', saveError)
