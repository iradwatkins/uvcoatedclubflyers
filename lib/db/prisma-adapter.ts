...
function buildSelectFields(select?: Partial<Record<keyof Airport, boolean>>): string {
  ...
  const fields = Object.keys(select).filter(
    (key) => select[key as keyof Airport]
  )
  ...
  async findMany(args: AirportFindManyArgs = {}): Promise<Partial<Airport>[]> {
    ...
    if (orderBy && Object.keys(orderBy).length > 0) {
      const orderFields = Object.entries(orderBy).map(([field, direction]) => {
        const dbField = field === 'isActive' ? 'is_active' : field === 'createdAt' ? 'created_at' : field === 'updatedAt' ? 'updated_at' : field
    ...
    if (select && Object.keys(select).length > 0) {
      return result.rows.map((row) => {
        ...
        Object.keys(select).forEach((key) => {
          const dbKey = key === 'isActive' ? 'is_active' : key === 'createdAt' ? 'created_at' : key === 'updatedAt' ? 'updated_at' : key
